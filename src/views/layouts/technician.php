...
<head>
    ...
    <title><?= htmlspecialchars($pageTitle ?? 'TuTécnico') ?></title>
...
<aside class="hidden md:flex flex-col h-full py-4 border-r border-outline-variant bg-surface-container-low shadow-sm w-64 flex-shrink-0 z-20">
    ...
    <div class="px-6 mb-6">
        ...
        <p class="text-xs text-on-surface-variant mt-1 pl-8">Panel del técnico</p>
    </div>
    ...
    <nav class="flex flex-col space-y-1 flex-1 px-2">
        <?php
        $navLinks = [
            'solicitudes' => ['href' => '/dashboard/tecnico/solicitudes', 'icon' => 'assignment', 'label' => 'Solicitudes'],
        ];
        ...
        <a href="/perfil" class="flex items-center px-4 py-3 rounded-lg text-on-surface-variant hover:bg-secondary-container transition-all">
            <span class="material-symbols-outlined mr-3 text-xl">account_circle</span>
            <span class="text-label-md font-label-md">Mi Perfil</span>
        </a>
    ...
    <div class="px-4 flex flex-col space-y-2 mt-4">
        <div class="flex items-center px-3 py-3 rounded-lg bg-surface border border-outline-variant/30">
            <?php
                $techName    = $_SESSION['user_name'] ?? 'T';
                $techParts   = explode(' ', trim($techName));
                $techInitial = mb_strtoupper(mb_substr($techParts[0], 0, 1));
                $techPhoto   = $_SESSION['user_photo'] ?? null;
            ?>
            <?php if ($techPhoto): ?>
                <img src="<?= htmlspecialchars($techPhoto) ?>"
            ...
            <?php else: ?>
                <div class="w-9 h-9 rounded-full bg-primary-container flex items-center justify-center mr-3 flex-shrink-0">
                    <span class="text-on-primary-container font-bold text-label-md"><?= $techInitial ?></span>
                </div>
            <?php endif; ?>
            <div class="flex flex-col flex-1 min-w-0">
                ...
                <span class="text-xs text-on-surface-variant">Técnico verificado</span>
            </div>
        </div>
...
<!-- Mobile header -->
<div class="md:hidden fixed top-0 left-0 right-0 z-30 bg-surface border-b border-outline-variant flex items-center justify-between px-4 h-14">
    <a href="/" class="flex items-center gap-2">
        <span class="material-symbols-outlined text-primary" style="font-variation-settings: 'FILL' 1;">home_repair_service</span>
        <span class="font-bold text-primary text-sm">TuTécnico</span>
    </a>
    <div class="flex items-center gap-3">
        <a href="/dashboard/tecnico/solicitudes" class="text-on-surface-variant">
            <span class="material-symbols-outlined">assignment</span>
        </a>
        <a href="/perfil" class="text-on-surface-variant">
...
<div class="flex-1 flex flex-col min-w-0 h-full overflow-hidden">
    <main class="flex-1 overflow-y-auto bg-background p-4 md:p-10 pt-16 md:pt-10">
